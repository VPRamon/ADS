...
func main() {
	...
	for{
		...
		//This is the channel we will use to send all the data from the listening routines to the main routine
		//that will be the one deciding.
		p := <- c

	//The code below does the corresponding actions for all the options when an Id is recived that are:
	//The recived id is bigger than the one we have, smaller or equal
		idInt := p.Id;
		ip := p.ip;
		port := p.port;
...
//if the id is bigger that the one recived , we ignore it.
		if parentId > idInt{

			fmt.Printf("Ignoring: %d ...\n",idInt);
			continue
			//if the id is bigger, we change from the wave we were and switch to this new recived.
			...
			// For all the neigbours we have, we wait for the message recived from them to take the
			...
			//If all the neighbours reply with the same id, then we send that id to our father.
